// pages/dashboard.jsx — Clinician Dashboard
...
export default function Dashboard() {
  ...
  return (
    <>
      <Head><title>Dashboard — MutaCure AR</title></Head>
        ...
        <div style={{ display:"flex", gap:24, alignItems:"flex-start" }}>
          ...
          <div style={{ flex:1, minWidth:0, display:"flex", flexDirection:"column", gap:20 }}>
              ...
              <div style={{ padding:"32px 36px", display:"flex", flexDirection:"column", justifyContent:"center" }}>
                ...
                {uploadedFile && (
                  <div style={{ marginTop:14, padding:"10px 14px", background:"#f0faf0", border:"1px solid #c8e6c9", borderRadius:9, display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <button onClick={handleUploadAndAnalyse} style={{ padding:"5px 12px", background:"#2d6a31", border:"none", borderRadius:7, color:"#fff", fontSize:11, fontWeight:600, cursor:"pointer" }}>Analyse →</button>
                    <button onClick={clearFile} style={{ background:"none", border:"none", cursor:"pointer", padding:2 }}><Icon n="x" s={14} c="#9aaa9a" /></button>
                  </div>
                )}
                {uploadError && <div style={{ marginTop:10, fontSize:11, color:"#e05c5c", fontFamily:"'DM Sans',sans-serif" }}>⚠ {uploadError}</div>}
              </div>
                    ...
                    return (
                      <div key={step.key} style={{ display:"flex", alignItems:"center", flex: i < PIPELINE_STEPS.length - 1 ? 1 : 0 }}>
                        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:5 }}>
                          ...
                          <span style={{ fontSize:10, color:done?"#2d7a31":active?"#1a2e1a":"#9aaa9a", fontWeight:active?600:400, whiteSpace:"nowrap", fontFamily:"'DM Sans',sans-serif" }}>{step.label}</span>
                        </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
              {[
                { label:"Gene", value:"TCF7L2", sub:"Type 2 Diabetes", extra:"4 New This Week", path:"/protein?target=PPARG&gene=TCF7L2&variant=rs7903146&risk=0.87", icon:"gene" },
                { label:"Generated Models", value:"12", sub:"2 New This Week", extra:"", path:"/protein", icon:"models" },
              ].map((card, i) => (
                  ...
                  <div style={{ padding:"20px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                      <div style={{ width:28, height:28, borderRadius:8, background:"#f0faf0", border:"1px solid #c8e6c9", display:"flex", alignItems:"center", justifyContent:"center" }}>
                        <Icon n={card.icon} s={14} c="#4caf50" />
                      </div>
                      <span style={{ fontSize:11, color:"#8aaa8a", fontFamily:"'DM Mono',monospace" }}>{card.label}</span>
                    </div>
              ...
              {BOTTOM_CARDS.map((card, ci) => (
                <div key={ci}
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:8 }}>
                    {card.tag && <span style={{ padding:"3px 9px", background:card.tagBg||"#f0f4f0", borderRadius:6, fontSize:11, color:card.tagColor||"#5a7a5a", fontFamily:"'DM Mono',monospace", fontWeight:500 }}>{card.tag}</span>}
            ...
            <div style={{ background:"#fff", borderRadius:14, border:"1px solid #e6ebe6", padding:18 }}>
              ...
              {RECENT_RIGHT.map((r,i) => (
                <div key={i}
                  ...
                  style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12, cursor:"pointer", borderRadius:6, padding:"8px 6px" }}
